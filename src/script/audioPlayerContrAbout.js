function audioPlayerContrAbout(track) {
  const audioPlayer = document.querySelector(
    '.audio-player__about .audio-player'
  );
  const audio = new Audio(track);
  const buttonPlay = document.querySelector(
    '.audio-player__about .toggle-play'
  );
  const timeline = audioPlayer.querySelector('.audio-player__about .timeline');
  const volumeSlider = audioPlayer.querySelector(
    '.audio-player__about .volume-slider'
  );
  let isPlay = true;
  ...
  volumeSlider.addEventListener('click', (e) => {
    ...
    audioPlayer.querySelector(
      '.audio-player__about .volume-percentage'
...
export default audioPlayerContrAbout;
